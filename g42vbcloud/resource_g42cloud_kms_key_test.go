...
func TestAccKmsKey_Basic(t *testing.T) {
	...
	var resourceName = "g42vbcloud_kms_key.key_1"
		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					...
					resource.TestCheckResourceAttr(resourceName, "region", G42VB_REGION_NAME),
				),
			},
			{
...
func testAccCheckKmsKeyDestroy(s *terraform.State) error {
	...
	kmsClient, err := config.KmsKeyV1Client(G42VB_REGION_NAME)
	if err != nil {
		return fmt.Errorf("Error creating G42VBCloud kms client: %s", err)
	}

	for _, rs := range s.RootModule().Resources {
		if rs.Type != "g42vbcloud_kms_key" {
			continue
		}
