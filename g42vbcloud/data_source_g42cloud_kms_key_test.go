...
func TestAccKmsKeyDataSource_Basic(t *testing.T) {
	...
	var datasourceName = "data.g42vbcloud_kms_key.key_1"
		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					...
					resource.TestCheckResourceAttr(datasourceName, "region", G42VB_REGION_NAME),
				),
			},
		},
	})
		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					testAccCheckKmsKeyDataSourceID(datasourceName),
					resource.TestCheckResourceAttr(datasourceName, "key_alias", keyAlias),
					resource.TestCheckResourceAttr(datasourceName, "enterprise_project_id", G42VB_ENTERPRISE_PROJECT_ID_TEST),
				),
			},
		},
	})
...
data "g42vbcloud_kms_key" "key_1" {
  key_alias = g42vbcloud_kms_key.key_1.key_alias
  key_id    = g42vbcloud_kms_key.key_1.id
  key_state = "2"
}
...
data "g42vbcloud_kms_key" "key_1" {
  ...
  key_description = "test description"
  key_state       = "2"
  enterprise_project_id = g42vbcloud_kms_key.key_1.enterprise_project_id
}
`, keyAlias, G42VB_ENTERPRISE_PROJECT_ID_TEST)
}
