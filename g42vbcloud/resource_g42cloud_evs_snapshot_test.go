...
func TestAccEvsSnapshotV2_basic(t *testing.T) {
	...
	resourceName := "g42vbcloud_evs_snapshot.test"
...
func testAccCheckEvsSnapshotV2Destroy(s *terraform.State) error {
	...
	evsClient, err := config.BlockStorageV2Client(G42VB_REGION_NAME)
	if err != nil {
		return fmt.Errorf("Error creating G42VBCloud EVS storage client: %s", err)
	}

	for _, rs := range s.RootModule().Resources {
		if rs.Type != "g42vbcloud_evs_snapshot" {
			continue
		}
...
resource "g42vbcloud_evs_snapshot" "test" {
