...
func TestAccLBV2L7Policy_basic(t *testing.T) {
	...
	resourceName := "g42vbcloud_lb_l7policy.l7policy_1"
...
func testAccCheckLBV2L7PolicyDestroy(s *terraform.State) error {
	...
	lbClient, err := config.ElbV2Client(G42VB_REGION_NAME)
	if err != nil {
		return fmt.Errorf("Error creating G42VBCloud load balancing client: %s", err)
	}

	for _, rs := range s.RootModule().Resources {
		if rs.Type != "g42vbcloud_lb_l7policy" {
			continue
		}
