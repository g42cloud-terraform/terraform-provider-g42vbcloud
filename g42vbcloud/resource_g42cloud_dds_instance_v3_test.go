...
func TestAccDDSV3Instance_basic(t *testing.T) {
	...
	resourceName := "g42vbcloud_dds_instance.instance"
...
func testAccCheckDDSV3InstanceDestroy(s *terraform.State) error {
	...
	client, err := config.DdsV3Client(G42VB_REGION_NAME)
	if err != nil {
		return fmt.Errorf("Error creating G42VBCloud DDS client: %s", err)
	}

	for _, rs := range s.RootModule().Resources {
		if rs.Type != "g42vbcloud_dds_instance" {
			continue
		}
...
resource "g42vbcloud_networking_secgroup" "secgroup_acc" {
  name = "%s"
}
...
resource "g42vbcloud_dds_instance" "instance" {
  ...
  security_group_id = g42vbcloud_networking_secgroup.secgroup_acc.id
  password          = "Test@123"
  mode              = "Sharding"
...
}`, rName, rName, G42VB_ENTERPRISE_PROJECT_ID_TEST)
}
