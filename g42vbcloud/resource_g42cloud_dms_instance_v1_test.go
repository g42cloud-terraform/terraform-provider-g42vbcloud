...
func TestAccDmsInstancesV1_Rabbitmq(t *testing.T) {
	...
	resourceName := "g42vbcloud_dms_instance.instance_1"
...
func testAccCheckDmsV1InstanceDestroy(s *terraform.State) error {
	...
	dmsClient, err := config.DmsV1Client(G42VB_REGION_NAME)
	if err != nil {
		return fmt.Errorf("Error creating G42VBCloud instance client: %s", err)
	}

	for _, rs := range s.RootModule().Resources {
		if rs.Type != "g42vbcloud_dms_instance" {
			continue
		}
...
func testAccCheckDmsV1InstanceExists(n string, instance instances.Instance) resource.TestCheckFunc {
	return func(s *terraform.State) error {
		...
		if err != nil {
			return fmt.Errorf("Error getting G42VBCloud instance: %s, err: %s", rs.Primary.ID, err)
		}
...
resource "g42vbcloud_networking_secgroup" "test" {
  name = "%s"
}
`, name)
}
...
resource "g42vbcloud_dms_instance" "instance_1" {
  ...
  subnet_id         = data.g42vbcloud_vpc_subnet.test.id
  security_group_id = g42vbcloud_networking_secgroup.test.id
  available_zones   = [data.g42vbcloud_dms_az.test.id]
...
resource "g42vbcloud_dms_instance" "instance_1" {
  ...
  product_id        = data.g42vbcloud_dms_product.product_1.id
  engine_version    = data.g42vbcloud_dms_product.product_1.version
  specification     = data.g42vbcloud_dms_product.product_1.bandwidth
  partition_num     = data.g42vbcloud_dms_product.product_1.partition_num
  storage_space     = data.g42vbcloud_dms_product.product_1.storage
  storage_spec_code = data.g42vbcloud_dms_product.product_1.storage_spec_code
