		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					testAccCheckSMNV2TopicExists("g42vbcloud_smn_topic.topic_1", &topic),
					resource.TestCheckResourceAttr(
						"g42vbcloud_smn_topic.topic_1", "name", rName),
					resource.TestCheckResourceAttr(
						"g42vbcloud_smn_topic.topic_1", "display_name",
						displayName),
				),
			},
		},
...
func testAccCheckSMNTopicV2Destroy(s *terraform.State) error {
	...
	for _, rs := range s.RootModule().Resources {
		if rs.Type != "g42vbcloud_smn_topic" {
			continue
		}
