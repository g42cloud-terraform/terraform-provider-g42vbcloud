...
func TestAccVpcBandWidthV2_basic(t *testing.T) {
	...
	resourceName := "g42vbcloud_vpc_bandwidth.test"
...
func testAccCheckVpcBandWidthV2Destroy(s *terraform.State) error {
	...
	networkingClient, err := config.NetworkingV1Client(G42VB_REGION_NAME)
	if err != nil {
		return fmt.Errorf("Error creating g42vbcloud networking client: %s", err)
	}

	for _, rs := range s.RootModule().Resources {
		if rs.Type != "g42vbcloud_vpc_bandwidth" {
			continue
		}
...
func testAccVpcBandWidthV2_basic(rName string, size int) string {
	return fmt.Sprintf(`
resource "g42vbcloud_vpc_bandwidth" "test" {
	name = "%s"
	size = "%d"
}
`, rName, size)
...
`, rName, size, G42VB_ENTERPRISE_PROJECT_ID_TEST)
}
