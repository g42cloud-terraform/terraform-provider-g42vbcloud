...
func TestAccRdsInstanceV3_basic(t *testing.T) {
	...
	resourceType := "g42vbcloud_rds_instance"
	resourceName := "g42vbcloud_rds_instance.test"
		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					testAccCheckRdsInstanceV3Exists(resourceName, &instance),
					resource.TestCheckResourceAttr(resourceName, "enterprise_project_id", G42VB_ENTERPRISE_PROJECT_ID_TEST),
				),
			},
		},
	})
...
func testAccCheckRdsInstanceV3Destroy(rsType string) resource.TestCheckFunc {
	return func(s *terraform.State) error {
		...
		client, err := config.RdsV3Client(G42VB_REGION_NAME)
		if err != nil {
			return fmt.Errorf("Error creating G42VBCloud rds client: %s", err)
		}
...
resource "g42vbcloud_rds_instance" "test" {
  name              = "%s"
  ...
  subnet_id         = g42vbcloud_vpc_subnet.test.id
  vpc_id            = g42vbcloud_vpc.test.id
  time_zone         = "UTC+08:00"
...
`, testAccRdsInstanceV3_base(name), name, G42VB_ENTERPRISE_PROJECT_ID_TEST)
}
...
resource "g42vbcloud_rds_instance" "test" {
  ...
  availability_zone   = [
    data.g42vbcloud_availability_zones.test.names[0],
    data.g42vbcloud_availability_zones.test.names[0],
  ]
