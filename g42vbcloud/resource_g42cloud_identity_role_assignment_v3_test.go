		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					testAccCheckIdentityV3RoleAssignmentExists("g42vbcloud_identity_role_assignment_v3.role_assignment_1", &role, &group, &project),
					resource.TestCheckResourceAttrPtr(
						"g42vbcloud_identity_role_assignment_v3.role_assignment_1", "project_id", &project.ID),
					resource.TestCheckResourceAttrPtr(
						"g42vbcloud_identity_role_assignment_v3.role_assignment_1", "group_id", &group.ID),
					resource.TestCheckResourceAttrPtr(
						"g42vbcloud_identity_role_assignment_v3.role_assignment_1", "role_id", &role.ID),
				),
			},
		},
	})
...
func testAccCheckIdentityV3RoleAssignmentDestroy(s *terraform.State) error {
	...
	for _, rs := range s.RootModule().Resources {
		if rs.Type != "g42vbcloud_identity_role_assignment_v3" {
			continue
		}
...
data "g42vbcloud_identity_role_v3" "role_1" {
  name = "ims_adm"
}

resource "g42vbcloud_identity_role_assignment_v3" "role_assignment_1" {
  group_id = "${g42vbcloud_identity_group_v3.group_1.id}"
  #project_id = "09338f629380276a2f95c0180c2bdb72"
  project_id = "0910fc31530026f82fd0c018a303517e"
  role_id = "${data.g42vbcloud_identity_role_v3.role_1.id}"
}
`
