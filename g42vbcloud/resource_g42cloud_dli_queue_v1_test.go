...
func testAccCheckDliQueueV1Destroy(s *terraform.State) error {
	...
	client, err := config.DliV1Client(G42VB_REGION_NAME)
	if err != nil {
		return fmt.Errorf("Error creating sdk client, err=%s", err)
	}

	for _, rs := range s.RootModule().Resources {
		if rs.Type != "g42vbcloud_dli_queue" {
			continue
		}
		...
		return fmt.Errorf("g42vbcloud_dli_queue still exists")
	}
...
func testAccCheckDliQueueV1Exists() resource.TestCheckFunc {
	return func(s *terraform.State) error {
		...
		rs, ok := s.RootModule().Resources["g42vbcloud_dli_queue.queue"]
		if !ok {
			return fmt.Errorf("Error checking g42vbcloud_dli_queue.queue exist, err=not found this resource")
		}

		_, err = fetchDliQueueV1ByListOnTest(rs, client)
		if err != nil {
			if strings.Index(err.Error(), "Error finding the resource by list api") != -1 {
				return fmt.Errorf("g42vbcloud_dli_queue is not exist")
			}
			return fmt.Errorf("Error checking g42vbcloud_dli_queue.queue exist, err=%s", err)
		}
		return nil
	}
}
