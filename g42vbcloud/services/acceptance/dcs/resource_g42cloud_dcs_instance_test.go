...
func getDcsResourceFunc(c *config.Config, state *terraform.ResourceState) (interface{}, error) {
	client, err := c.DcsV2Client(acceptance.G42VB_REGION_NAME)
	if err != nil {
...
func TestAccDcsInstances_basic(t *testing.T) {
	...
	resourceName := "g42vbcloud_dcs_instance.instance_1"
		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					...
					resource.TestCheckResourceAttr(resourceName, "enterprise_project_id",
						acceptance.G42VB_ENTERPRISE_PROJECT_ID_TEST),
				),
			},
		},
	})
...
resource "g42vbcloud_dcs_instance" "instance_1" {
  ...
  availability_zones = [data.g42vbcloud_availability_zones.test.names[0]]
...
}`, instanceName, acceptance.G42VB_ENTERPRISE_PROJECT_ID_TEST)
}
