...
func TestAccCESAlarmRule_basic(t *testing.T) {
	...
	resourceName := "g42vbcloud_ces_alarmrule.alarmrule_1"
...
func testCESAlarmRuleDestroy(s *terraform.State) error {
	...
	networkingClient, err := config.CesV1Client(G42VB_REGION_NAME)
	if err != nil {
		return fmt.Errorf("Error creating G42VBCloud ces client: %s", err)
	}

	for _, rs := range s.RootModule().Resources {
		if rs.Type != "g42vbcloud_ces_alarmrule" {
			continue
		}
...
resource "g42vbcloud_compute_instance" "vm_1" {
  name              = "ecs-%s"
...
resource "g42vbcloud_ces_alarmrule" "alarmrule_1" {
  alarm_name           = "rule-%s"
  ...
  metric {
    ...
    dimensions {
      name  = "instance_id"
      value = g42vbcloud_compute_instance.vm_1.id
    }
  }
  ...
  alarm_actions {
    type              = "notification"
    notification_list = [
      g42vbcloud_smn_topic.topic_1.topic_urn
    ]
  }
}
