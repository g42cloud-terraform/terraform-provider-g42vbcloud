		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					testAccCheckSMNV2SubscriptionExists("g42vbcloud_smn_subscription.subscription_1", &subscription1),
					testAccCheckSMNV2SubscriptionExists("g42vbcloud_smn_subscription.subscription_2", &subscription2),
					resource.TestCheckResourceAttr(
						"g42vbcloud_smn_subscription.subscription_1", "endpoint",
						"mailtest@gmail.com"),
					resource.TestCheckResourceAttr(
						"g42vbcloud_smn_subscription.subscription_2", "endpoint",
						"13600000000"),
				),
			},
		},
...
func testAccCheckSMNSubscriptionV2Destroy(s *terraform.State) error {
	...
	smnClient, err := config.SmnV2Client(G42VB_REGION_NAME)
	if err != nil {
		return fmt.Errorf("Error creating G42VBCloud smn: %s", err)
	}
	var subscription *subscriptions.SubscriptionGet
	for _, rs := range s.RootModule().Resources {
		if rs.Type != "g42vbcloud_smn_subscription" {
			continue
		}
...
func testAccCheckSMNV2SubscriptionExists(n string, subscription *subscriptions.SubscriptionGet) resource.TestCheckFunc {
	return func(s *terraform.State) error {
		...
		smnClient, err := config.SmnV2Client(G42VB_REGION_NAME)
		if err != nil {
			return fmt.Errorf("Error creating G42VBCloud smn client: %s", err)
		}
