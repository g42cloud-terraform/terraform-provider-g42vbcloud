		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					testAccCheckDNSV2ZoneExists("g42vbcloud_dns_zone.zone_1", &zone),
					resource.TestCheckResourceAttr(
						"g42vbcloud_dns_zone.zone_1", "description", "a zone"),
				),
			},
			{
				ResourceName:      "g42vbcloud_dns_zone.zone_1",
				ImportState:       true,
		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					...
					resource.TestMatchResourceAttr(
						"g42vbcloud_dns_zone.zone_1", "ttl", regexp.MustCompile("^[0-9]+$")),
				),
			},
		},
	})
...
func testAccCheckDNSV2ZoneDestroy(s *terraform.State) error {
	...
	if err != nil {
		return fmt.Errorf("Error creating G42VBCloud DNS client: %s", err)
	}

	for _, rs := range s.RootModule().Resources {
		if rs.Type != "g42vbcloud_dns_zone" {
			continue
		}
