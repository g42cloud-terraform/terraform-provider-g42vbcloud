		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					testAccCheckComputeV2VolumeAttachExists("g42vbcloud_compute_volume_attach.va_1", &va),
				),
			},
			{
				ResourceName:      "g42vbcloud_compute_volume_attach.va_1",
				ImportState:       true,
...
func testAccCheckComputeV2VolumeAttachDestroy(s *terraform.State) error {
	...
	for _, rs := range s.RootModule().Resources {
		if rs.Type != "g42vbcloud_compute_volume_attach" {
			continue
		}
...
resource "g42vbcloud_compute_volume_attach" "va_1" {
  instance_id = g42vbcloud_compute_instance.instance_1.id
  volume_id = g42vbcloud_evs_volume.test.id
}
