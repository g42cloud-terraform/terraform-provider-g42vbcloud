		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					testAccCheckLBV2MemberExists("g42vbcloud_lb_member.member_1", &member_1),
					testAccCheckLBV2MemberExists("g42vbcloud_lb_member.member_2", &member_2),
				),
			},
			{
				...
				ExpectNonEmptyPlan: true, // Because admin_state_up remains false.
				Check: resource.ComposeTestCheckFunc(
					resource.TestCheckResourceAttr("g42vbcloud_lb_member.member_1", "weight", "10"),
					resource.TestCheckResourceAttr("g42vbcloud_lb_member.member_2", "weight", "15"),
				),
			},
		},
	})
...
func testAccCheckLBV2MemberDestroy(s *terraform.State) error {
	...
	for _, rs := range s.RootModule().Resources {
		if rs.Type != "g42vbcloud_lb_member" {
			continue
		}
...
resource "g42vbcloud_lb_member" "member_1" {
  address       = "192.168.0.10"
  protocol_port = 8080
  pool_id       = g42vbcloud_lb_pool.pool_1.id
  subnet_id     = data.g42vbcloud_vpc_subnet.test.subnet_id
...
resource "g42vbcloud_lb_member" "member_2" {
