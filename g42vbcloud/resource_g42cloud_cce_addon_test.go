...
func TestAccCCEAddonV3_basic(t *testing.T) {
	...
	resourceName := "g42vbcloud_cce_addon.test"
...
func testAccCheckCCEAddonV3Destroy(s *terraform.State) error {
	...
	cceClient, err := config.CceAddonV3Client(G42VB_REGION_NAME)
	if err != nil {
		return fmt.Errorf("Error creating G42VBCloud CCE Addon client: %s", err)
	}
	...
	for _, rs := range s.RootModule().Resources {
		...
		if rs.Type != "g42vbcloud_cce_addon" {
			continue
		}
...
func testAccCCEAddonImportStateIdFunc() resource.ImportStateIdFunc {
	return func(s *terraform.State) (string, error) {
		...
		for _, rs := range s.RootModule().Resources {
			if rs.Type == "g42vbcloud_cce_cluster" {
				clusterID = rs.Primary.ID
			} else if rs.Type == "g42vbcloud_cce_addon" {
...
resource "g42vbcloud_cce_node" "test" {
...
resource "g42vbcloud_cce_addon" "test" {
  cluster_id    = g42vbcloud_cce_cluster.test.id
  version       = "1.15.10"
  template_name = "autoscaler"
  depends_on    = [g42vbcloud_cce_node.test]
}
