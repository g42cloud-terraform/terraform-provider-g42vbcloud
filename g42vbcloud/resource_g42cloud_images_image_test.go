...
func TestAccImsImage_basic(t *testing.T) {
	...
	resourceName := "g42vbcloud_images_image.test"
...
func testAccCheckImsImageDestroy(s *terraform.State) error {
	...
	imageClient, err := config.ImageV2Client(G42VB_REGION_NAME)
	if err != nil {
		return fmt.Errorf("Error creating G42VBCloud Image: %s", err)
	}

	for _, rs := range s.RootModule().Resources {
		if rs.Type != "g42vbcloud_images_image" {
			continue
		}
