...
func TestAccRdsReadReplicaInstance_basic(t *testing.T) {
	...
	resourceType := "g42vbcloud_rds_read_replica_instance"
	resourceName := "g42vbcloud_rds_read_replica_instance.test"
...
resource "g42vbcloud_rds_read_replica_instance" "test" {
  name                = "%s"
  flavor              = "rds.pg.c6.large.4.rr"
  primary_instance_id = g42vbcloud_rds_instance.test.id
  availability_zone   = data.g42vbcloud_availability_zones.test.names[0]
...
`, testAccReadRdsReplicaInstanceV3_base(name), name, G42VB_ENTERPRISE_PROJECT_ID_TEST)
}
