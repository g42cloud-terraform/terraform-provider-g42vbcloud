		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					testAccCheckComputeV2InterfaceAttachExists("g42vbcloud_compute_interface_attach.ai_1", &ai),
...
func testAccCheckComputeV2InterfaceAttachDestroy(s *terraform.State) error {
	...
	for _, rs := range s.RootModule().Resources {
		if rs.Type != "g42vbcloud_compute_interface_attach" {
			continue
		}
...
resource "g42vbcloud_compute_interface_attach" "ai_1" {
  instance_id = g42vbcloud_compute_instance.instance_1.id
  network_id =  data.g42vbcloud_vpc_subnet.test.id
