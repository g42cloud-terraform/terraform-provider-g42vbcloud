...
func TestAccNetworkACL_basic(t *testing.T) {
	...
	resourceKey := "g42vbcloud_network_acl.fw_1"
	...
	resource.ParallelTest(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			...
			},
			{
				...
				Check: resource.ComposeTestCheckFunc(
					testAccCheckNetworkACLExists("g42vbcloud_network_acl.fw_1", &fwGroup),
...
func testAccCheckNetworkACLDestroy(s *terraform.State) error {
	...
	fwClient, err := config.FwV2Client(G42VB_REGION_NAME)
	if err != nil {
		return fmt.Errorf("Error creating G42VBCloud fw client: %s", err)
	}
	for _, rs := range s.RootModule().Resources {
		if rs.Type != "g42vbcloud_network_acl" {
			continue
		}
...
resource "g42vbcloud_vpc_subnet" "subnet_2" {
	name = "%s_subnet_2"
...
resource "g42vbcloud_network_acl" "fw_1" {
  ...
  subnets = [g42vbcloud_vpc_subnet.subnet_1.id,
      g42vbcloud_vpc_subnet.subnet_2.id]
}
`, testAccNetworkACLRules(name), name)
}
