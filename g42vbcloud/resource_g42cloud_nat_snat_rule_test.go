...
func TestAccNatSnatRule_basic(t *testing.T) {
	...
	resourceName := "g42vbcloud_nat_snat_rule.snat_1"
		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					testAccCheckNatV2GatewayExists("g42vbcloud_nat_gateway.nat_1"),
...
func testAccCheckNatV2SnatRuleDestroy(s *terraform.State) error {
	...
	natClient, err := config.NatGatewayClient(G42VB_REGION_NAME)
	if err != nil {
		return fmt.Errorf("Error creating G42VBCloud nat client: %s", err)
	}

	for _, rs := range s.RootModule().Resources {
		if rs.Type != "g42vbcloud_nat_snat_rule" {
			continue
		}
...
resource "g42vbcloud_nat_snat_rule" "snat_1" {
  ...
  network_id     = g42vbcloud_vpc_subnet.subnet_1.id
  floating_ip_id = g42vbcloud_vpc_eip.eip_1.id
}
