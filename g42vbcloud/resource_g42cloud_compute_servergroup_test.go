		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					testAccCheckComputeV2ServerGroupExists("g42vbcloud_compute_servergroup.sg_1", &sg),
				),
			},
			{
				ResourceName:      "g42vbcloud_compute_servergroup.sg_1",
				ImportState:       true,
		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					testAccCheckComputeV2ServerGroupExists("g42vbcloud_compute_servergroup.sg_1", &sg),
					testAccCheckComputeV2InstanceExists("g42vbcloud_compute_instance.instance_1", &instance),
...
func testAccCheckComputeV2ServerGroupDestroy(s *terraform.State) error {
	...
	for _, rs := range s.RootModule().Resources {
		if rs.Type != "g42vbcloud_compute_servergroup" {
			continue
		}
...
func testAccComputeV2ServerGroup_basic(rName string) string {
	return fmt.Sprintf(`
resource "g42vbcloud_compute_servergroup" "sg_1" {
  name = "%s"
  policies = ["affinity"]
}
`, rName)
...
resource "g42vbcloud_compute_instance" "instance_1" {
  ...
  scheduler_hints {
    group = g42vbcloud_compute_servergroup.sg_1.id
  }
  network {
