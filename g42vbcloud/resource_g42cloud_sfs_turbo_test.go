...
func TestAccSFSTurbo_basic(t *testing.T) {
	...
	resourceName := "g42vbcloud_sfs_turbo.sfs-turbo1"
...
func testAccCheckSFSTurboDestroy(s *terraform.State) error {
	...
	sfsClient, err := config.SfsV1Client(G42VB_REGION_NAME)
	if err != nil {
		return fmt.Errorf("Error creating G42cloud sfs turbo client: %s", err)
	}

	for _, rs := range s.RootModule().Resources {
		if rs.Type != "g42vbcloud_sfs_turbo" {
			continue
		}
...
resource "g42vbcloud_sfs_turbo" "sfs-turbo1" {
