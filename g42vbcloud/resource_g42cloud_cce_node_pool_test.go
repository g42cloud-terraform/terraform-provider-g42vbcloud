...
func TestAccCCENodePool_basic(t *testing.T) {
	...
	resourceName := "g42vbcloud_cce_node_pool.test"
	//clusterName here is used to provide the cluster id to fetch cce node pool.
	clusterName := "g42vbcloud_cce_cluster.test"
...
func testAccCheckCCENodePoolDestroy(s *terraform.State) error {
	...
	cceClient, err := config.CceV3Client(G42VB_REGION_NAME)
	if err != nil {
		return fmt.Errorf("Error creating G42VBCloud CCE client: %s", err)
	}
	...
	for _, rs := range s.RootModule().Resources {
		if rs.Type == "g42vbcloud_cce_cluster" {
			clusterId = rs.Primary.ID
		}

		if rs.Type == "g42vbcloud_cce_node_pool" {
...
func testAccCCENodePoolImportStateIdFunc() resource.ImportStateIdFunc {
	return func(s *terraform.State) (string, error) {
		cluster, ok := s.RootModule().Resources["g42vbcloud_cce_cluster.test"]
		if !ok {
			return "", fmt.Errorf("Cluster not found: %s", cluster)
		}
		nodePool, ok := s.RootModule().Resources["g42vbcloud_cce_node_pool.test"]
		if !ok {
...
resource "g42vbcloud_cce_node_pool" "test" {
  cluster_id         = g42vbcloud_cce_cluster.test.id
  name               = "%s"
  os                 = "EulerOS 2.5"
  flavor_id          = "s6.large.2"
  initial_node_count = 1
  availability_zone  = data.g42vbcloud_availability_zones.test.names[0]
  key_pair           = g42vbcloud_compute_keypair.test.name
  scall_enable      = false
  min_node_count    = 0
  max_node_count    = 0
