		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					testAccCheckIdentityV3GroupMembershipExists("g42vbcloud_identity_group_membership_v3.membership_1", []string{userName}),
				),
			},
			{
				Config: testAccIdentityV3GroupMembership_update(groupName, userName, userName2),
				Check: resource.ComposeTestCheckFunc(
					testAccCheckIdentityV3GroupMembershipExists("g42vbcloud_identity_group_membership_v3.membership_1", []string{userName, userName2}),
				),
			},
			{
				Config: testAccIdentityV3GroupMembership_updatedown(groupName, userName2),
				Check: resource.ComposeTestCheckFunc(
					testAccCheckIdentityV3GroupMembershipExists("g42vbcloud_identity_group_membership_v3.membership_1", []string{userName2}),
				),
			},
		},
	})
...
func testAccCheckIdentityV3GroupMembershipDestroy(s *terraform.State) error {
	...
	for _, rs := range s.RootModule().Resources {
		if rs.Type != "g42vbcloud_identity_group_membership_v3" {
			continue
		}
...
func testAccIdentityV3GroupMembership_basic(groupName, userName string) string {
	return fmt.Sprintf(`
    resource "g42vbcloud_identity_group_v3" "group_1" {
      name = "%s"
    }
    ...
    resource "g42vbcloud_identity_group_membership_v3" "membership_1" {
        group = "${g42vbcloud_identity_group_v3.group_1.id}"
        users = ["${g42vbcloud_identity_user_v3.user_1.id}"]
    }
  `, groupName, userName)
}
...
func testAccIdentityV3GroupMembership_update(groupName, userName string, userName2 string) string {
    ...
    resource "g42vbcloud_identity_user_v3" "user_2" {
      name = "%s"
      password = "password123@#"
      enabled = true
    }
    ...
    resource "g42vbcloud_identity_group_membership_v3" "membership_1" {
        group = "${g42vbcloud_identity_group_v3.group_1.id}"
        users = ["${g42vbcloud_identity_user_v3.user_1.id}",
                "${g42vbcloud_identity_user_v3.user_2.id}"]
    }
...
func testAccIdentityV3GroupMembership_updatedown(groupName, userName string) string {
    ...
    resource "g42vbcloud_identity_group_membership_v3" "membership_1" {
        group = "${g42vbcloud_identity_group_v3.group_1.id}"
        users = ["${g42vbcloud_identity_user_v3.user_2.id}"]
    }
  `, groupName, userName)
}
