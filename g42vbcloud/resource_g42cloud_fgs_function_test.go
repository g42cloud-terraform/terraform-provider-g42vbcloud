...
func TestAccFgsV2Function_basic(t *testing.T) {
	...
	resourceName := "g42vbcloud_fgs_function.test"
...
func testAccCheckFgsV2FunctionDestroy(s *terraform.State) error {
	...
	fgsClient, err := config.FgsV2Client(G42VB_REGION_NAME)
	if err != nil {
		return fmt.Errorf("Error creating G42VBCloud FGS V2 client: %s", err)
	}

	for _, rs := range s.RootModule().Resources {
		if rs.Type != "g42vbcloud_fgs_function" {
			continue
		}
...
func testAccFgsV2Function_basic(rName string) string {
	return fmt.Sprintf(`
resource "g42vbcloud_fgs_function" "test" {
  name        = "%s"
  app         = "default"
...
`, rName, G42VB_ENTERPRISE_PROJECT_ID_TEST)
}
