...
func TestAccComputeV2Instance_basic(t *testing.T) {
	...
	resourceName := "g42vbcloud_compute_instance.test"
...
func testAccCheckComputeV2InstanceDestroy(s *terraform.State) error {
	...
	for _, rs := range s.RootModule().Resources {
		if rs.Type != "g42vbcloud_compute_instance" {
			continue
		}
...
func testAccCheckComputeV2InstanceTags(
	...
	return func(s *terraform.State) error {
		config := testAccProvider.Meta().(*config.Config)
		client, err := config.ComputeV1Client(G42VB_REGION_NAME)
		if err != nil {
			return fmt.Errorf("Error creating G42VBCloud compute v1 client: %s", err)
		}
