		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					testAccCheckIdentityV3GroupExists("g42vbcloud_identity_group_v3.group_1", &group),
					resource.TestCheckResourceAttrPtr(
						"g42vbcloud_identity_group_v3.group_1", "name", &group.Name),
					resource.TestCheckResourceAttrPtr(
						"g42vbcloud_identity_group_v3.group_1", "description", &group.Description),
					resource.TestCheckResourceAttrPtr(
						"g42vbcloud_identity_group_v3.group_1", "domain_id", &group.DomainID),
				),
			},
			{
...
func testAccCheckIdentityV3GroupDestroy(s *terraform.State) error {
	...
	for _, rs := range s.RootModule().Resources {
		if rs.Type != "g42vbcloud_identity_group_v3" {
			continue
		}
