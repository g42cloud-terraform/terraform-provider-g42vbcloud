...
func TestAccLBV2LoadBalancer_basic(t *testing.T) {
	...
	resourceName := "g42vbcloud_lb_loadbalancer.loadbalancer_1"
		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					...
					testAccCheckNetworkingV2SecGroupExists(
						"g42vbcloud_networking_secgroup.secgroup_1", &sg_1),
					testAccCheckNetworkingV2SecGroupExists(
						"g42vbcloud_networking_secgroup.secgroup_1", &sg_2),
			...
			},
			{
				...
				Check: resource.ComposeTestCheckFunc(
					...
					testAccCheckNetworkingV2SecGroupExists(
						"g42vbcloud_networking_secgroup.secgroup_2", &sg_1),
					testAccCheckNetworkingV2SecGroupExists(
						"g42vbcloud_networking_secgroup.secgroup_2", &sg_2),
					testAccCheckLBV2LoadBalancerHasSecGroup(&lb, &sg_1),
...
func testAccCheckLBV2LoadBalancerDestroy(s *terraform.State) error {
	...
	for _, rs := range s.RootModule().Resources {
		if rs.Type != "g42vbcloud_lb_loadbalancer" {
			continue
		}
...
func testAccCheckLBV2LoadBalancerExists(
	...
	return func(s *terraform.State) error {
		...
		elbClient, err := config.ElbV2Client(G42VB_REGION_NAME)
		if err != nil {
			return fmt.Errorf("Error creating G42VBCloud networking client: %s", err)
		}
...
resource "g42vbcloud_lb_loadbalancer" "loadbalancer_1" {
  ...
  vip_subnet_id      = data.g42vbcloud_vpc_subnet.test.subnet_id
  security_group_ids = [
    g42vbcloud_networking_secgroup.secgroup_1.id
  ]
}
...
resource "g42vbcloud_lb_loadbalancer" "loadbalancer_1" {
  ...
  security_group_ids = [
    g42vbcloud_networking_secgroup.secgroup_1.id,
    g42vbcloud_networking_secgroup.secgroup_2.id
  ]
}
`, rNameSecg1, rNameSecg2, rName)
}
