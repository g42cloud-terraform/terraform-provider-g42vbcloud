...
func TestAccObsBucket_basic(t *testing.T) {
	...
	resourceName := "g42vbcloud_obs_bucket.bucket"
		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					...
					resource.TestCheckResourceAttr(
						resourceName, "region", G42VB_REGION_NAME),
				),
			},
			{
		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					...
					resource.TestCheckResourceAttr(
						resourceName, "bucket", testAccObsBucketName(rInt)),
					resource.TestCheckResourceAttr(
						resourceName, "enterprise_project_id", G42VB_ENTERPRISE_PROJECT_ID_TEST),
				),
			},
		},
	})
...
func testAccCheckObsBucketDestroy(s *terraform.State) error {
	...
	for _, rs := range s.RootModule().Resources {
		if rs.Type != "g42vbcloud_obs_bucket" {
			continue
		}

		_, err := obsClient.HeadBucket(rs.Primary.ID)
		if err == nil {
			return fmt.Errorf("G42VBCloud OBS Bucket %s still exists", rs.Primary.ID)
		}
	}
	return nil
}
...
func testAccCheckObsBucketExists(n string) resource.TestCheckFunc {
	return func(s *terraform.State) error {
		...
		_, err = obsClient.HeadBucket(rs.Primary.ID)
		if err != nil {
			return fmt.Errorf("G42VBCloud OBS Bucket not found: %v", err)
		}
		return nil
	}
}
...
func testAccObsBucketDomainName(randInt int) string {
	return fmt.Sprintf("tf-test-bucket-%d.obs.%s.g42vbcloud.com", randInt, G42VB_REGION_NAME)
}
...
`, randInt, G42VB_ENTERPRISE_PROJECT_ID_TEST)
}
...
func testAccObsBucketConfigWithLogging(randInt int) string {
	return fmt.Sprintf(`
resource "g42vbcloud_obs_bucket" "log_bucket" {
...
resource "g42vbcloud_obs_bucket" "bucket" {
	...
	logging {
		target_bucket = g42vbcloud_obs_bucket.log_bucket.id
		target_prefix = "log/"
	}
}
`, randInt, randInt)
