...
```hcl
resource "g42vbcloud_kms_key" "key" {
  key_alias       = "key_1"
...
resource "g42vbcloud_rds_instance" "instance" {
  ...
  volume {
    type               = "ULTRAHIGH"
    size               = 100
    disk_encryption_id = g42vbcloud_kms_key.key.id
  }
  backup_strategy {
    start_time = "08:00-09:00"
    keep_days  = 1
...
```
$ terraform import g42vbcloud_rds_instance.instance_1 7117d38e-4c8f-4624-a505-bd96b97d024c
```

But due to some attrubutes missing from the API response, it's required to ignore changes as below.

```
resource "g42vbcloud_rds_instance" "instance_1" {
  ...
